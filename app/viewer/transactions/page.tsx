...
// Interface định nghĩa cấu trúc dữ liệu giao dịch
...
// Interface định nghĩa cấu trúc dữ liệu cửa hàng
interface Store {
  id: string
  name: string
  storeCode: string
...
export default function ViewerTransactionsPage() {
  ...
  // Load stores và transactions khi component mount
  useEffect(() => { fetchStores() }, [])

  // Reload transactions khi filter hoặc page thay đổi
  useEffect(() => { fetchTransactions() }, [page, storeId, type, dateFrom, dateTo, searchCode])

  function getToken() { return localStorage.getItem('token') || '' }

  // Lấy danh sách cửa hàng để hiển thị trong dropdown filter
  ...
  // Lấy danh sách giao dịch theo filter hiện tại
  ...
  return (
    <div style={{ padding: 32 }}>
      ...
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          ...
          <p style={{ color: '#999', marginTop: 4 }}>
            ...
            Tổng tiền: <strong style={{ color: '#2563eb' }}>{formatMoney(totalRevenue)}</strong>
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        {/* Nút xóa filter khi đang có filter */}
            ...
            <tbody>
              {transactions.map((tx, i) => (
                <tr key={tx.id} style={{
                  ...
                  <Td>
                    <span style={{
                      color: tx.type === 'ADJUSTMENT' && tx.balanceAfter > tx.balanceBefore
                        ? '#16a34a' : '#dc2626',
                      fontWeight: 700,
                    }}>
                  ...
                  <Td>
                    <span style={{
                      ...
                      padding: '4px 8px', // Tăng nhẹ lên 4px nhìn cái nhãn sẽ cân đối hơn
                      borderRadius: 6, 
                      fontSize: 12, 
                      fontWeight: 600,
                      // Kích hoạt flex để icon và chữ thẳng hàng hoàn hảo
                      display: 'inline-flex',
                      alignItems: 'center',
                      gap: 6,
                    }}>
                      {tx.type === 'ADJUSTMENT' ? (
                        <>
                          <Coins size={14} strokeWidth={2} style={{ flexShrink: 0 }} />
                          <span>Điều chỉnh</span>
                        </>
                      ) : (
                        <>
                          <CreditCard size={14} strokeWidth={2} style={{ flexShrink: 0 }} />
                          <span>Thanh toán</span>
                        </>
                      )}
                    </span>
                  </Td>
                  <Td style={{ color: '#666', fontSize: 12 }}>{tx.reason || '—'}</Td>
