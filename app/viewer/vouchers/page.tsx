...
// Interface định nghĩa cấu trúc dữ liệu thẻ voucher
interface Voucher {
  id: string
  voucherCode: string
  holderName: string
...
// Interface định nghĩa cấu trúc dữ liệu partner
interface Partner {
  id: string
  name: string
}

export default function ViewerVouchersPage() {
  ...
  // Load partners và vouchers khi component mount
  useEffect(() => { fetchPartners() }, [])

  // Reload vouchers khi filter hoặc page thay đổi
  useEffect(() => { fetchVouchers() }, [page, partnerId, status])

  function getToken() { return localStorage.getItem('token') || '' }

  // Lấy danh sách vouchers theo filter
  ...
  // Lấy danh sách partners để hiển thị trong dropdown filter
    ...
    <div style={{ padding: 32 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: '#999', marginTop: 4 }}>
          Tổng cộng <strong style={{ color: '#1a1a1a' }}>{total}</strong> thẻ
        </p>
      </div>
      ...
      {/* Bảng danh sách thẻ — chỉ xem, không có nút điều chỉnh */}
